...
// * utilizzata per implementare controlli di integrita. Poichè il protocollo prevede che l'unico messaggio
// * firmato con campi mutevoli hop per hop e il datagramma DOWNSTREAM, il campo mutevole (ossia hop count)
// * viene azzerato prima di passare alla callback utente e poi ripristinato. in questo modo non è necessario
// * ricalcolare la firma ad ogni ricezione, sebbene non verrà garantita l'integrità su tale campo
// *
...
//#else
//
//	//se nella pfwd c'è il mio indirizzo, sto ricevendo una pfwd generata da me, e qua è fatt e loop
